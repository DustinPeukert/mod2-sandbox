...
//Write a function called findInfo that takes in a range and a specific peak.
//The function should return the elevation of the specified peak in the specified range.
...
function findHighGainRoutes(range, peak) {
  let allRoutes = fourteeners[range].peaks[peak].routes;
  let numOfRoutes = allRoutes.reduce((acc, route) => {
    if (route.gain >= 3000) {
      acc++;
    }
    return acc;
  }, 0)
  return `This peak has ${numOfRoutes} routes that have at least 3000 feet of gain.`;
}
...
function findTotal(range, peak, datapoint) {
  let allRoutes = fourteeners[range].peaks[peak].routes;
  let total = allRoutes.reduce((acc, route) => {
    acc += route[datapoint];
    return acc;
  }, 0)
  return `If you hike every route for this peak you will achieve ${total} total ${datapoint}!`
}
